...
plot1 <- ggplot(df_clean, aes(x = AgeGroup, y = AJT_Accuracy)) +
...
```{r}
# example of how to use regressions to generate significance markers in plots

model <- glmer(AJT_Accuracy ~ Condition * AgeGroup + (1 | PID) + (1 | Item), 
               data = df_clean, 
               family = 'binomial',
               control = glmerControl(optimizer = "bobyqa"))

# Get estimated marginal means and contrasts
em <- emmeans(model, ~ Condition | AgeGroup)
contrast_results <- pairs(em, adjust = "none")  # Change adjust method if needed

# Prepare data for significance markers
...
ggplot(df_clean, aes(x = AgeGroup, y = AJT_Accuracy)) +
  stat_summary(aes(fill = Condition), fun = mean, geom = 'bar', 
               position = position_dodge(), alpha = 0.7, color = "black") +
  stat_summary(aes(fill = Condition), fun.data = function(x) {
    data.frame(y = mean(x), 
...
```
```{r}
# example of how to use regression to conduct t test to chance

df_sub <- df_clean %>%
  filter(AgeGroup == "3", Condition == "Age")

# Fit logistic regression: intercept-only model
model_3yo_age <- glmer(AJT_Accuracy ~ 1 + (1 | PID) + (1 | Item), 
                       data = df_sub, 
                       family = 'binomial',
                       control = glmerControl(optimizer = "bobyqa"))

# Summary of the model
summary(model_3yo_age)
```
```{r}
#example of how to use regression to conduct comparison between conditions- but do I use the previous model or this one to compare 3s in Age to chance?

df_3yo <- df_clean %>%
  filter(AgeGroup == "3")

# Fit logistic regression: Condition as predictor
model_3yo_cond <- glmer(AJT_Accuracy ~ Condition + (1 | PID) + (1 | Item),
                        data = df_3yo,
                        family = 'binomial',
                        control = glmerControl(optimizer = "bobyqa"))

# Model summary
summary(model_3yo_cond)
```
...
test_results_noagesamesize <- df_clean %>%
  filter(Condition == "No Age", Congruence == "Same") %>%
  group_by(AgeGroup) %>%
  summarise(
    p_value = t.test(AJT_Accuracy, mu = 0.5)$p.value,
    .groups = "drop"
  ) %>%
  mutate(
    annotation = case_when(
    ...
    y_position = 0.95,  # adjust as needed
    xmin = as.numeric(as.factor(AgeGroup)) - 0.2,
    xmax = as.numeric(as.factor(AgeGroup)) + 0.2,
    group = AgeGroup
  )

noagesamesizeplot <- ggplot(noagesamesize_df, aes(x = AgeGroup, y = MeanAccuracy)) +
...
test_results <- wide_df %>%
  group_by(Condition) %>%
  summarise(
    p_value = t.test(`0`, `1`, paired = TRUE)$p.value,
    .groups = "drop"
  ) %>%
  mutate(
    y_position = case_when(
      Condition == "Age" ~ 0.88,
      Condition == "No Age" ~ 0.7
    ),
    xmin = case_when(
      Condition == "Age" ~ 0.8,
...
plot3 <- ggplot(df_clean, aes(x = Condition, y = AJT_Accuracy)) +
  ...
  scale_x_discrete(labels = c("Age" = "Age Provided", "No Age" = "No Age Provided")) +
  geom_hline(yintercept = 0.5, linetype = 'dashed', color = 'black') +
  geom_signif(
    data = test_results,
    aes(xmin = xmin, xmax = xmax, annotations = annotation, y_position = y_position, group = group),
...
ggsave("plot3.png", plot = plot3, width = 8, height = 5, dpi = 300)
...
df_filtered <- df_clean %>%
  filter(Condition == "No Age", Ratio %in% c("3 vs 5", "3 vs 9"))
t.test(AJT_Accuracy ~ Ratio, data = df_filtered)
```

```{r}
# correct way to do t test
agg_age_3yo <- df_clean %>%
  filter(AgeGroup == "3", Condition == "Age") %>%
  group_by(PID) %>%
  summarise(prop_correct = mean(AJT_Accuracy), .groups = "drop")

# Step 2: One-sample t-test against chance (0.5)
t.test(agg_age_3yo$prop_correct, mu = 0.5)
```
