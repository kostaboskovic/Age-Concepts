...
print(results_table)
```
```{r, GLM table summary formatting}
library(tidyr)
...
results_table_clean %>%
  ...
  mutate(across(starts_with("Age") | starts_with("Congruence") | starts_with("Condition") | 
                starts_with("LG_Accuracy") | starts_with("AMT_Accuracy"), 
                ~ replace_na(., ""))) %>%  # Replace NA with blank
  
  # Round the numeric coefficient statistics to two decimal places and add significance stars
  mutate(across(
    where(is.numeric),  # Apply to numeric columns only
    ~ ifelse(. != "", paste0(round(., 2), " ", stars.p(tidy(glmer1)$p.value[match(cur_column(), names(tidy(glmer1)))])), ""))) %>%  # Round to 2 decimals and add stars
  ...
  tab_spanner(
    label = "Model",
    columns = Model,
    id = "model_spanner"  # Give this spanner a unique ID
  ) %>%
  tab_spanner(
    label = html("Coefficient statistics (&#946;)"),
    columns = c(
      `Age (months)`, `Incongruent`, `Same Size`,
      `Condition: No Numerical Age`, `Later-Greater Task Accuracy`, `Autobiographical Memory Task Accuracy`
    ),
    id = "coef_spanner"  # Unique ID for coefficient spanner
  ) %>%
  tab_spanner(
    label = "Summary statistics",
    columns = c(AIC, LogLik),
    id = "summary_spanner"  # Unique ID for summary statistics spanner
  ) %>%
  cols_label(
    Model = "Model",
    AIC = "AIC",
  ...
  tab_footnote(
    footnote = "Note: Reference levels are as follows: Size Congruence = Congruent and Condition = Numerical Age Provided.",
    locations = cells_title(groups = "title")  # Footnote associated with the title
  ) %>%
  tab_footnote(
    footnote = "Significance codes: `***` p < 0.001, `**` p < 0.01, `*` p < 0.05.",
    locations = cells_title(groups = "subtitle")  # Footnote associated with the subtitle
  ) %>%
  tab_options(
