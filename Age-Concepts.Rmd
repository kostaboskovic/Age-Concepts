...
print_emm_contrast <- function(contrast_df, contrast_label, filter_level = NULL, digits = 2) {
  ...
  contrast_df <- contrast_df %>%
    filter(if_any(everything(), ~grepl(filter_level, as.character(.), fixed = TRUE)))
}
  row <- contrast_df %>%
...
```{r, plot showing Age Judgment Task accuracy for age group and condition}
summary_df <- df_clean %>%
  group_by(AgeGroup, Condition) %>%
  summarise(
    mean_accuracy = mean(AJT_Accuracy, na.rm = TRUE),
    sem = sd(AJT_Accuracy, na.rm = TRUE) / sqrt(n())
  )

ggplot(summary_df, aes(x = AgeGroup, y = mean_accuracy, fill = Condition)) +
  geom_col(
  ...
  geom_errorbar(
    aes(ymin = mean_accuracy - sem, ymax = mean_accuracy + sem),
    position = position_dodge(width = 0.9),
    width = 0.2
  ) +
  geom_hline(
