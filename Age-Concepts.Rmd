...
noagesamesizeplot <- ggplot(noagesamesize_df, aes(x = AgeGroup, y = MeanAccuracy)) +
  ...
  labs(
    x = "Age Group (Years)",
    y = "Mean AJT Accuracy",
    title = "AJT Performance for 'No Age' / 'Same' Congruence Trials"
  ) +
  theme_minimal(base_size = 14) +
...
#note to self: fixed the 1 3 vs 7 in the no age condition which had an extra white space, doesn't affect anything else but this ratio analysis so only need to import the data again if decide to report any ratio analysis
```
