...
df_clean$AgeGroup <- factor(df_clean$AgeGroup, levels = c(3, 4, 5))
df_clean <- df %>%
  filter(Task == 'AJT')
```
...
cor.test(participant_means$mean_AJT, participant_means$mean_LG)
...
AJT_summary_condition <- df_clean %>%
  ...
  summarise(mean_AJT = mean(AJT_Accuracy, na.rm = TRUE), .groups = "drop")

AJT_summary_stats <- AJT_summary_condition %>%
  group_by(AgeGroup, Condition) %>%
  summarise(
    m_AJT = mean(mean_AJT),  # different name
    sd_AJT = sd(mean_AJT),
    .groups = "drop"
  )
...
```{r, Later Greater Task descriptives and}
df_LG <- df %>%
  filter(Task == 'LG')
...
LG_participant_means <- df_LG %>%
  group_by(PID, AgeGroup, Ratio) %>%
  summarise(mean_LG = mean(LG_Accuracy, na.rm = TRUE), .groups = "drop")

LG_summary_by_age_ratio <- LG_participant_means %>%
  group_by(AgeGroup, Ratio) %>%
  summarise(
    m_LG = mean(mean_LG),
    se_LG = sd(mean_LG) / sqrt(n()),
    .groups = "drop"
  )


LGplot <- ggplot(LG_summary_by_age_ratio, aes(x = AgeGroup, y = m_LG, fill = Ratio)) +
  geom_col(position = position_dodge(0.9), width = 0.8) +
  geom_errorbar(aes(ymin = m_LG - se_LG, ymax = m_LG + se_LG),
  ...
  scale_fill_manual(values = c(
    "3 vs 5" = "#D0DCE5",  # very light slate blue
"3 vs 7" = "#A6BDD7",  # soft blue-gray
"3 vs 9" = "#7FA6C3",  # muted medium blue
"5 vs 7" = "#548ABF",  # slate blue
"5 vs 9" = "#2A6DAF",  # deeper blue
"7 vs 9" = "#0F4C81"   # dark navy blue
  )) +
...
ggsave("LGplot.png", plot = LGplot, width = 8, height = 6, dpi = 300)
...
```{r, if wanted to show comparisons to chance in plot1}
#chance_markers_df_plot1 <- tibble::tibble(
 # AgeGroup = c("3", "3", "4", "4", "5", "5"),
  #Condition = c("Age", "No Age", "Age", "No Age", "Age", "No Age"),
  #p_value = c(
   # age3_age_condition_t_test_result$p.value,
    #age3_noage_condition_t_test_result$p.value,
    #age4_age_condition_t_test_result$p.value,
    #age4_noage_condition_t_test_result$p.value,
    #age5_age_condition_t_test_result$p.value,
    #age5_noage_condition_t_test_result$p.value
  #)
#) %>%
 # mutate(
  #  annotation = case_when(
   #   p_value < 0.001 ~ "***",
    ...
    #),
 #   xpos = case_when(
  #    AgeGroup == "3" & Condition == "Age" ~ 0.8,
   #   AgeGroup == "3" & Condition == "No Age" ~ 1.2,
    #  AgeGroup == "4" & Condition == "Age" ~ 1.8,
     # AgeGroup == "4" & Condition == "No Age" ~ 2.2,
      #AgeGroup == "5" & Condition == "Age" ~ 2.8,
      #AgeGroup == "5" & Condition == "No Age" ~ 3.2
    #),
    #ypos = case_when(
     # AgeGroup == "3" & Condition == "Age" ~ 0.64,
      #AgeGroup == "3" & Condition == "No Age" ~ 0.58,
      #AgeGroup == "4" & Condition == "Age" ~ 0.81,
      #AgeGroup == "4" & Condition == "No Age" ~ 0.61,
      #AgeGroup == "5" & Condition == "Age" ~ 0.96,
      #AgeGroup == "5" & Condition == "No Age" ~ 0.74
    #)
  #)
```


```{r, plot of Age Judgment Task accuracy by condition and age group}

contrast_df_agecondition <- as.data.frame(age_condition_int)

contrast_df_agecondition <- contrast_df_agecondition %>%
  mutate(
    annotation = case_when(
      p.value < 0.001 ~ "***",
      p.value < 0.01 ~ "**",
...
# Plot
plot1 <- ggplot(AJT_summary_condition, aes(x = AgeGroup, y = mean_AJT, fill = Condition)) +
  stat_summary(
    fun = mean,
    geom = "bar",
  ...
  scale_fill_manual(
    ...
    labels = c("Age" = "Age Provided", "No Age" = "No Age Provided")
  ) +
...
ggsave("plot1.png", plot = plot1, width = 8, height = 5, dpi = 300)
...
contrast_df_congruencecondition <- as.data.frame(congruence_condition_int)

contrast_df_congruencecondition <- contrast_df_congruencecondition %>%
  mutate(
    y_position = case_when(
...
plot2 <- ggplot(df_clean, aes(x = Congruence, y = AJT_Accuracy)) +
  ...
  labs(
    x = "Congruence of Size and Age Cues",
    y = "Mean Accuracy",
    fill = "Numerical Age Condition"
  ) +
  scale_fill_manual(
  ...
  scale_x_discrete(labels = c("Same" = "Same Size")) +
  theme_minimal(base_size = 14) +
  theme(
    panel.grid = element_blank(),
    axis.line = element_line(color = "black"),
  ...
  geom_hline(yintercept = 0.5, linetype = 'dashed', color = 'black') +
    geom_signif(data = contrast_df_congruencecondition,
...
ggsave("plot2.png", plot = plot2, width = 8, height = 5, dpi = 300)
...
# 1. Compute participant-level means
AJT_summary_congruence <- df_clean %>%
  group_by(PID, Congruence, Condition) %>%
  summarise(mean_AJT = mean(AJT_Accuracy, na.rm = TRUE), .groups = "drop")

# 2. Compute group means and SEMs from participant means
AJT_summary_congruence_group <- AJT_summary_congruence %>%
  group_by(Congruence, Condition) %>%
  summarise(
    m_AJT = mean(mean_AJT),
    se_AJT = sd(mean_AJT) / sqrt(n()),
    .groups = "drop"
  )

# 3. Create the plot with SEM error bars
plot2 <- ggplot(AJT_summary_congruence_group, aes(x = Congruence, y = m_AJT, fill = Condition)) +
  geom_bar(stat = "identity", position = position_dodge(0.9), color = "black", alpha = 0.7) +
  geom_errorbar(aes(ymin = m_AJT - se_AJT, ymax = m_AJT + se_AJT),
...
```{r, t tests and plot for Age Judgment Task performance in No Age condition and Same size trials}

df_noagesamesize <- df_clean %>%
  filter(Condition == "No Age", Congruence == "Same")

noagesamesize_participant_means <- df_noagesamesize %>%
  group_by(PID, AgeGroup) %>%
  summarise(mean_AJT = mean(AJT_Accuracy, na.rm = TRUE), .groups = "drop")

noagesamesize_3yo <- t.test(noagesamesize_participant_means %>% filter(AgeGroup == "3") %>% pull(mean_AJT), mu = 0.5)
noagesamesize_3yo

noagesamesize_4yo <- t.test(noagesamesize_participant_means %>% filter(AgeGroup == "4") %>% pull(mean_AJT), mu = 0.5)
noagesamesize_4yo

noagesamesize_5yo <- t.test(noagesamesize_participant_means %>% filter(AgeGroup == "5") %>% pull(mean_AJT), mu = 0.5)
noagesamesize_5yo

summary_noagesamesize <- noagesamesize_participant_means %>%
  group_by(AgeGroup) %>%
  summarise(
    MeanAccuracy = mean(mean_AJT),
    SD = sd(mean_AJT),
    SE = SD / sqrt(n()),
    .groups = "drop"
  )

test_results_noagesamesize <- noagesamesize_participant_means %>%
  group_by(AgeGroup) %>%
  summarise(
    p_value = t.test(mean_AJT, mu = 0.5)$p.value,
    .groups = "drop"
  ) %>%
  mutate(
    annotation = case_when(
...
noagesamesizeplot <- ggplot(summary_noagesamesize, aes(x = AgeGroup, y = MeanAccuracy)) +
  geom_bar(stat = "identity", fill = "powderblue", color = "black", width = 0.6) +
  geom_errorbar(aes(ymin = MeanAccuracy - SE, ymax = MeanAccuracy + SE), width = 0.2) +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "black") +
  geom_text(
    data = test_results_noagesamesize,
    aes(x = AgeGroup, y = y_position, label = annotation),
    size = 5
  ) +
  labs(
    x = "Age Group (Years)",
...
ggsave("noagesamesizeplot.png", plot = noagesamesizeplot, width = 8, height = 5, dpi = 300)
...
summary_df <- df_clean %>%
  ...
  summarise(
    ...
    SE = sd(AJT_Accuracy, na.rm = TRUE) / sqrt(n()),
    .groups = "drop"
  )

ggplot(summary_df, aes(x = factor(Ratio), y = MeanAccuracy, fill = Condition)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), color = "black", alpha = 0.7) +
  geom_errorbar(
    aes(ymin = MeanAccuracy - SE, ymax = MeanAccuracy + SE),
  ...
  scale_fill_manual(
    values = c("Age" = "lightblue", "No Age" = "white"),
    labels = c("Age Provided", "No Age Provided")
  ) +
  labs(
    x = "Age Ratio (Younger/Older)",
    y = "Mean AJT Accuracy",
    fill = "Condition"
  ) +
  theme_minimal(base_size = 14) +
